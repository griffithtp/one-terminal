...
│   ├── src/
│   │   ├── index.ts                ← CLI entry — routes create / upgrade / new-widget
│   │   ├── create/                 ← scaffold a new workspace (variant-aware)
│   │   ├── new-widget/             ← `new-widget` subcommand (wrapped by `npm run create-widget` in scaffolds)
│   │   ├── upgrade/                ← migrate an existing workspace (variant-aware)
│   │   └── merge/                  ← config/dep merge helpers
│   ├── templates/                  ← GENERATED by extract-templates.ts
│   │   ├── shared/                 ← common to all variants
│   │   ├── enterprise/             ← full-stack-only files
│   │   └── standalone/             ← slim-variant-only files
│   ├── overlay-overrides/          ← hand-authored variant-specific files
│   │   └── standalone/             ← Cargo.toml, package.json, widgets.config.json, etc.
│   ├── widget-template/            ← embedded template used by `new-widget`
│   └── versions.json               ← migration manifest (bundled + published)
├── scripts/
│   ├── extract-templates.ts        ← derives templates/ from live apps + overlay-overrides
...
│   └── test-scaffold.ts            ← smoke-tests both variants + new-widget
└── .github/workflows/
...
### Overlay overrides (hand-authored)

Files under `packages/create-one-terminal/overlay-overrides/<overlay>/` are copied verbatim by the extractor into `templates/<overlay>/`. These are EJS templates (still rendered at scaffold time) for files that have no live source equivalent — primarily the slim Standalone `Cargo.toml.ejs`, `package.json.ejs`, `widgets.config.json.ejs`, and the standalone `terminal.config.json.ejs` override. They are the only hand-authored files in the template tree.
...
## Phase 9 — Standalone vs Enterprise Variants

Full design in [docs/plans/standalone-vs-enterprise-plans.md](docs/plans/standalone-vs-enterprise-plans.md).

- [x] **A** — Restructure `templates/` into `shared/` / `enterprise/` / `standalone/` overlays. Extractor classifies each `SOURCES` entry; `render.ts` walks `shared/` then the variant overlay (variant-overlay-wins). `static-manifest.json` records `{path, overlay}` per entry.
- [x] **B** — Add `variant` prompt (default `standalone`) and `externalFdc3AgentUrl` prompt. New framework app `apps/sample-widget/` (Standalone-only sample). Hand-authored Standalone files under `packages/create-one-terminal/overlay-overrides/standalone/` for slim Cargo.toml, package.json, widgets.config.json, terminal.config.json.
- [x] **C** — Terminal `WidgetSource` abstraction. New Rust `widgets` module + `wm_list_apps` Tauri command. Frontend `loadWidgetRegistry()` helper centralises the source choice. `WidgetSourceKind` enum + `OT_WIDGET_SOURCE` / `OT_WIDGETS_CONFIG_PATH` env overrides.
- [x] **D** — Two surfaces for adding widgets, both variant-aware:
  - Framework subcommand `create-one-terminal new-widget <name>` (used via `npx`; embedded template at `packages/create-one-terminal/widget-template/*.ejs`, programmatic `createWidget(cwd, spec)` API).
  - In-workspace `npm run create-widget` → invokes `node scripts/new-widget.mjs`, a self-contained ~250-line Node-stdlib-only script (no `@clack/prompts`, no `ejs` — uses `node:readline` and `<%= var %>` regex substitution). The script + its `scripts/widget-template/*.tpl` files are hand-authored under `packages/create-one-terminal/overlay-overrides/shared/scripts/` and ship in both Standalone and Enterprise scaffolds. Zero external dependencies — works offline. The smoke test exercises this entrypoint to catch divergence from the framework subcommand.
- [x] **E** — FDC3 client mode. New `fdc3.agentUrl` config + `OT_FDC3_AGENT_URL` env override. `panel_init_script` injects `window.OT_FDC3_AGENT_URL` into every panel webview at creation (wm_open, pool prewarm, pool replenish, panel restore). `fdc3-plugin.js` URL resolution updated; empty URL short-circuits to `NoAgentConfigured`.
- [x] **F** — Migration wizard variant-awareness. `appliesTo: "both" | "standalone" | "enterprise"` field on `MigrationSpec`. `create-migration` infers `appliesTo` from the overlay prefix and strips that prefix from the migration's `target`. `buildChain` filters migrations by the workspace variant before applying. `detect.ts` reads the variant from `oneTerminal.variant`, falling back to filesystem inference for pre-variant scaffolds.

Verified end-to-end via `npm run test:scaffold` (both variants → cargo check + npm install + new-widget; Enterprise additionally builds App Directory).
