...
- [x] **D** — `create-one-terminal new-widget <name>` subcommand (wrapped by `npm run create-widget <name>` in scaffolded workspaces). Embedded template at `widget-template/`. Programmatic `createWidget(cwd, spec)` API for tests. Variant-aware registration: Standalone appends to `widgets.config.json`; Enterprise prints curl (or POSTs if `OT_APPD_TOKEN` is set). Scaffolded `package.json` lists `create-one-terminal` as an `optionalDependency` so the bin resolves locally without `npx`.
