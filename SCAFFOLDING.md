...
│   ├── src/
│   │   ├── index.ts                ← CLI entry — routes create / upgrade / add-widget
...
│   ├── widget-template/            ← embedded template used by `add-widget`
...
│   └── test-scaffold.ts            ← smoke-tests both variants + add-widget
└── .github/workflows/
...
- [x] **D** — `create-one-terminal add-widget <name>` subcommand. Embedded template at `widget-template/`. Programmatic `createWidget(cwd, spec)` API for tests. Variant-aware registration: Standalone appends to `widgets.config.json`; Enterprise prints curl (or POSTs if `OT_APPD_TOKEN` is set).
...
Verified end-to-end via `npm run test:scaffold` (both variants → cargo check + npm install + add-widget; Enterprise additionally builds App Directory).
