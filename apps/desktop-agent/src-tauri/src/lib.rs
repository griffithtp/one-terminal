...
use broker::{
    ChannelInfo, ChannelManager, IntentHandlerInfo, IntentRegistry,
    WindowHandle, WindowManager, new_uuid,
};
...
fn cda_raise_intent_from_dashboard(
    ...
    let resolved = match &target_instance_id {
        Some(tid) => {
            if window_manager.get(tid).is_some() { Some(tid.clone()) } else { None }
        }
...
async fn cda_launch_app(
    ...
    let appd: &AppRecord = match records.iter().find(|r| r.app_id == app_id) {
        ...
        None => {
            appd_owned = AppRecord {
                ...
                description: None,
                version: None,
                details: Some(app_dir::AppDetails { url: Some(app_url.clone()) }),
                categories: Vec::new(),
                interop: None,
    ...
    let plan = plan_launch(appd, &LaunchContext::Standalone, installed.as_ref(), &manifests)?;
...
async fn execute_plan(
    ...
    match plan {
        ...
        LaunchPlan::SpawnTauriHost { runtime_folder, url, title } => {
        ...
        LaunchPlan::SpawnElectronHost { binding, url, title } => {
        ...
        LaunchPlan::SpawnBinary { binary_name, env_vars } => {
...
fn locate_wm_binary() -> Result<std::path::PathBuf, String> {
    ...
    let exe = std::env::current_exe().map_err(|e| format!("current_exe: {e}"))?;
    let dir = exe.parent().ok_or_else(|| "exe has no parent dir".to_string())?;
    let name = if cfg!(windows) { "one-terminal.exe" } else { "one-terminal" };
    let candidate = dir.join(name);
    if !candidate.exists() {
        return Err(format!(
...
async fn build_tray_menu(app: &AppHandle) -> tauri::Result<tauri::menu::Menu<tauri::Wry>> {
    ...
    if apps.is_empty() {
        ...
        for record in &apps {
            ...
            let item = MenuItemBuilder::with_id(format!("launch:{}", record.app_id), label)
                .build(app)?;
            sub = sub.item(&item);
        }
    }
...
async fn launch_app_from_tray(app: AppHandle, app_id: String) {
    ...
    let installed = if let Some(binding) = effective_binding {
        ...
        match app.state::<EngineRuntimeStore>().ensure(&binding, &snapshot, &app).await {
            Ok(e) => Some(e),
            Err(e) => {
    ...
    let manifests = app.state::<EngineRuntimeStore>().plugins().to_vec();
    match plan_launch(&record, &LaunchContext::Standalone, installed.as_ref(), &manifests) {
        Ok(plan) => {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(dacp::start(pr, app_handle, dacp_instance_id, ports.dacp_bridge));
